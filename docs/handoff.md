...
- After deploying the read-only role refresh, repeat a logged-in TOP request after the 60-second boundary and confirm it remains below 1.5 seconds.
...
- Related cache/auth/UI regression bundle after the `v14` cache-generation and read-only role-refresh changes: 72 passed (`test_auth_phase_migration.py`, `test_playwright_password_login.py`, `test_supabase_auth_stripe_migration.py`, `test_race_detail_ui_facts.py`, `test_race_detail_page_prewarm.py`, `test_admin_data_status.py`, `test_race_detail_data_schedule.py`, `test_public_roi_page.py`, `test_base_template_optional_endpoints.py`).
