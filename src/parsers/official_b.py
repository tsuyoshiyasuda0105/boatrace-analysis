...
# ============================================================
# NOTE: B file の大会名キーワードからの推測は限界あり。
#   - 「日刊スポーツ杯」等の名前は G1 でも一般戦でも使われる
#   - 過剰検出を避けるため、明確に G1 級 と分かる固有名詞のみ採用
# 最も確実な grade source は Open API (2025-07 以降のみ供給)。
# 過去日 (2022-2024) は LZH 推測のためデータ品質に限界あり (G1/G3 を
# 一般戦と誤判定する可能性が残る)。
GRADE_KEYWORDS: list[tuple[int, list[str]]] = [
    ...
    (2, ["Ｇ１", "周年記念", "グランドチャンピオン", "海の祭典",
         "モーターボート記念", "地区選手権", "ＢＢＣトーナメント"]),
    # G2
    (3, ["Ｇ２", "クイーンズクライマックス", "レディースチャレンジ",
         "ヤングダービー", "マスターズチャンピオン"]),
    # G3 (キーワード判定難しい、ヤング/レディース/マスターズ等は誤検出多い
    # ので主要なもののみに絞る)
    (4, ["Ｇ３", "ヴィーナス"]),
]
